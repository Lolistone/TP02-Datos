...
Pudimos destacar la importancia de comenzar con un analisis exploratorio de las letras L y A, lo que nos permitió notar y diferenciar aquellos pixeles que presentan mayor varianza entre las dos letras, para luego mejorar la performance utilizando estos atributos. Una vez hecho esto, tambien observamos que la perfomance baja al aumentar la cantidad de vecinos. Por lo tanto, podemos concluir que tomando un modelo que tenga entre 3 y 10 pixeles con alta varianza y con k = 1, tendriamos un modelo eficiente con una perfomance entre $0.99 y 1.00$

\subsection{Modelo de Clasificación Multiclase}

Utilizando el metodo de validacion cruzada con K-Folding nos premitió obtener una estimación de performance del árbol de deicision más realista. Decimos que fue mas realista debido a la forma que tiene el metodo de dividir nuestro conjunto de entrenamiento, logrando que cada parte del conjunto de datos se utilice como datos de prueba (una vez) sin pasar por la fase de entrenamiento. Logrando asi concluir que a partir de la altura 10, el modelo mantiene una precisión mayor a 0.9.

Para obtener los mejores hiperparámetros de nuestro modelo realizamos Grid Search para obtener aquellos que mejoren su rendimiento. Tomamos en consideracion los hiperparámetros: profundidad máxima del árbol y el criterio de división. Concluyendo que el modelo final es un árbol de altura 10 con el criterio entropia, lo cual demostramos poniendo a prueba el modelo frente a los datos de test que separamos al principio de todo el proceso. 
